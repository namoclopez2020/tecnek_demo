...
class  Media {
  ...
  public $empresaPath = SITE_ROOT.DS.'..'.DS.'uploads/empresa';
 ...
 /*--------------------------------------------------------------*/
  /* Function for Process empresa image
  /*--------------------------------------------------------------*/
 public function process_empresa($id){
    ...
    if(!is_writable($this->empresaPath)){
        $this->errors[] = $this->empresaPath." Debe tener permisos de escritura";
        return false;
      }
    if(!$id){
      $this->errors[] = " ID de usuario ausente.";
    ...
    $new_name = randString(8).$id.'.' . end($ext);
    $this->fileName = $new_name;
    if($this->empresa_image_destroy($id))
    {
    if(move_uploaded_file($this->fileTempPath,$this->empresaPath.'/'.$this->fileName))
       {

         if($this->update_empresaImg($id)){
           unset($this->fileTempPath);
           return true;
         }
 ...
 /*--------------------------------------------------------------*/
 /* Function for Update empresa image
 /*--------------------------------------------------------------*/
  private function update_empresaImg($id){
     global $db;
      $sql = "UPDATE sucursales SET";
      $sql .=" image_path='{$db->escape($this->fileName)}'";
 ...
 /*--------------------------------------------------------------*/
  private function update_userImg($id){
   ...
   /*--------------------------------------------------------------*/
 /* Function for Delete old image from empresa
 /*--------------------------------------------------------------*/
  public function empresa_image_destroy($id){
     $image = find_by_id('sucursales',$id);
     if($image['image_path'] === 'no_image.jpg')
     {
       return true;
     } else {
       unlink($this->empresaPath.'/'.$image['image_path']);
       return true;
     }
